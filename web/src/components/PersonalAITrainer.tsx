...
List each movement that requires loading and give them an EXACT number based on their lift PRs. Example: "Deadlifts: Use 185lb (that's 65% of your 285lb 1RM - perfect for this workout style)." If a movement is in the ZERO LOGGED DATA list above, prescribe the ramp protocol (empty bar / lightest implement, small jumps while form stays solid) - never a fixed working weight.

**PACING & REP SCHEME STRATEGY:**
For scored conditioning pieces, give a specific pacing target. For AMRAP: target rounds/hour and how to break up reps (e.g., "Break the wall balls into sets of 10 from the start"). For For Time: target finish time and when to push/rest. For EMOMs: work-to-rest ratio goals. Be SPECIFIC with numbers. For strength/lift work: the PROGRAM is the authority on execution - repeat only the sets, reps, tempo, and rest that are WRITTEN in the workout. NEVER invent execution details the program didn't specify (no added tempo counts, pauses, "explosive up", or exact rest times). If the program leaves rest/tempo unstated, say "rest enough that form stays solid" and move on - do not fill the gap with invented precision. If this section would only restate the written program, keep it to one or two sentences.
...
- The programmed workout is the ONLY authority on HOW to perform the work. Never add or change execution details - tempo counts, pauses, bar speed, rest intervals, warm-up set counts - beyond what the program wrote. You coach loads, effort, pacing of scored pieces, safety, and mindset AROUND the program - not a different way to do it.
...
- SAFETY OVERRIDES EVERYTHING: never recommend above 100% of their max, and respect rep-max physiology (2 reps=95%, 3=92%, 5=87%, 8=80%, 10=75%, 12=70% of 1RM). If the programmed load exceeds what their numbers support, SAY SO and give the safe load instead. On any max attempt or baseline test: start light and build gradually, stop the moment form starts to slip - 100% effort, never 110% - and set safeties; they train alone. On movements with NO data, explicitly advise a spotter if at all possible - no spotter means rack safeties or the dumbbell version, staying well shy of failure. Frame tests as a snapshot of today, not a judgment: Oddo builds them up from here, safely.
...
Respond in a confident, direct coach tone. This advice will be saved and shown every time they view this workout, so make it count.`;
      ...
      if (offender) {
        const retryText = await chatCompletion({
          messages: [
            { role: "system", content: "You are an experienced CrossFit coach providing personalized workout advice. You only coach the workout you are given - you never invent extra movements or change the programmed rep scheme, and your percentage/weight math is always correct." },
            { role: "user", content: `${prompt}\n\nYOUR PREVIOUS DRAFT NAMED A WORKING WEIGHT FOR "${offender}", which has ZERO logged data - that is unsafe and forbidden. Rewrite the full advice now: every zero-data movement gets ONLY the ramp protocol (empty bar / lightest implement, small jumps while form stays solid, log where you land) with no number above 45lb anywhere in its prescription.` }
          ],
          temperature: 0.2,
          onDelta: (textSoFar) => {
            setIsStreaming(true);
