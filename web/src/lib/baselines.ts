...
1. The FIRST 1-2 WEEKS of any new plan MUST be a BASELINE BLOCK: schedule the MISSING battery tests above (max 2-3 tests per week, spread across days, never two max-effort tests in one session). Title each component EXACTLY "Baseline: <test name>" (e.g., "Baseline: Back Squat") and use the standard prescription for that test. Every baseline component's description stays practical: start light, stop if form starts to slip - 100% effort, never 110% - spotter if possible (no spotter = rack safeties or the DB version, staying shy of failure). The reassurance framing - this is just a snapshot of today, good or bad; the coach builds them up from here, safely - goes ONCE in each baseline day's "reason", never repeated inside components.
