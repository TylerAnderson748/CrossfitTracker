...
final class AppStore: ObservableObject {
    ...
    /// Repair missing default groups for a gym
    /// Checks which default groups exist and creates any that are missing
    func repairDefaultGroupsForGym(gymId: String, ownerId: String, completion: @escaping (Int, String?) -> Void) {
        print("🔧 [repairDefaultGroups] Checking for missing default groups in gym \(gymId)")

        // First, load existing groups for this gym
        loadGroupsForGym(gymId: gymId) { existingGroups, error in
            if let error = error {
                completion(0, error)
                return
            }

            // Define the default groups that should exist
            let defaultGroupNames = ["Members", "Competition Athletes", "Weight Training Athletes"]

            // Find which groups are missing
            let existingNames = Set(existingGroups.map { $0.name })
            let missingNames = defaultGroupNames.filter { !existingNames.contains($0) }

            print("🔧 [repairDefaultGroups] Existing groups: \(existingNames)")
            print("🔧 [repairDefaultGroups] Missing groups: \(missingNames)")

            if missingNames.isEmpty {
                print("✅ [repairDefaultGroups] All default groups exist")
                DispatchQueue.main.async {
                    completion(0, nil)
                }
                return
            ...
            var createdCount = 0
            var lastError: String?
            let totalToCreate = missingNames.count
            ...
            for name in missingNames {
                ...
                self.createGroup(group) { savedGroup, error in
                    if let error = error {
                        print("❌ [repairDefaultGroups] Error creating \(name): \(error)")
                        lastError = error
                    } else {
                        print("✅ [repairDefaultGroups] Created missing group: \(name)")
                        createdCount += 1
                    }

                    // Check if we're done
                    let processed = createdCount + (lastError != nil ? 1 : 0)
                    if processed >= totalToCreate || createdCount == totalToCreate {
                        DispatchQueue.main.async {
                            completion(createdCount, lastError)
                        }
                    }
                }
            }
